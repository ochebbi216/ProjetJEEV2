    ...
    <main class="container mt-4">
        ...
        <div class="row">
                    ...
                    <tbody>
                        <c:choose>
                            <c:when test="${empty paniers}">
                                <tr>
                                    <td colspan="5" style="color: red;">Your cart was empty. Go to the menu and add what you want.</td>
                                </tr>
                            </c:when>
                            <c:otherwise>
                                <c:forEach var="panier" items="${paniers}">
                                    <c:if test="${panier.userid == userId}">
                                        <c:set var="discountedPrice" value="${panier.pizza.prixBase - (panier.pizza.prixBase * panier.pizza.discountPercentage / 100)}" />
                                        <c:set var="totalPrice" value="${discountedPrice * panier.quantite}" />
                                        <tr data-panier-id="${panier.panierId}" data-prix-total="${String.format('%.2f', totalPrice)}">
                                            <td><img style="margin-bottom:25px;" src="${panier.pizza.image}" alt="${panier.pizza.nom}" width=100 height=80 /></td>
                                            <td><br> ${panier.pizza.nom}</td>
                                            <td><br> <c:out value="x ${panier.quantite}" /></td>
                                            <td><br>
                                                <c:out value="${String.format('%.2f', totalPrice)} DT" />
                                            </td>
                                            <td><br>
                                                <button class="btn " onclick="deletePanier(${panier.panierId});">
                                                    <script src="https://cdn.lordicon.com/lordicon.js"></script>
                                                    <lord-icon style="margin-bottom:15px;" src="https://cdn.lordicon.com/drxwpfop.json" trigger="hover" colors="primary:#e83a30,secondary:#545454" style="width:35px;height:35px"></lord-icon>
                                                </button>
                                            </td>
                                        </tr>
                                    </c:if>
            ...
            <div id="deliveryForm" class="col-4">
                ...
                <form method="POST">
                    <div class="mb-3">
                        <label for="numTel" class="form-label">Phone Number</label> <input type="text" class="form-control" id="numTel" name="numTel" placeholder="Enter your phone number" required>
                    </div>
                    <div class="mb-3">
                        <label for="adresseLivraison" class="form-label">Delivery Address</label> <input type="text" class="form-control" id="adresseLivraison" name="adresseLivraison" placeholder="Enter your delivery address" required>
                    </div>
                </form>
            </div>
        </div>
